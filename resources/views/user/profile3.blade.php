            ...
            <table class="table-auto w-full border-collapse border border-gray-200 rounded-lg">
                ...
                <tbody>
                    ...
                    <tr class="bg-gray-100">
                        <td colspan="2" class="px-4 py-2 border border-gray-200 text-left font-bold">No Transaksi: 12345</td> <!-- Display Transaction ID -->
                        <td class="px-4 py-2 border border-gray-200 text-center">
                            <button type="button" id="simulate" class="px-2 py-1 text-white bg-blue-600 hover:bg-blue-700 rounded-md"
                    ...
                    <tr class="bg-gray-50">
                        <td class="px-4 py-2 border border-gray-200">{{ $item->product->product_name }}</td>
                        <td class="px-4 py-2 border text-center border-gray-200">{{ $transaction->created_at }}</td>
                        <td class="px-4 py-2 border text-center border-gray-200">{{ $transaction->status_pengiriman }}</td>
                    </tr>
                    @endforeach
                    @endforeach
                </tbody>
...
<script>
    ...
    this.deliveryMarker = L.marker(startPoint, { icon: motorIcon }).addTo(this.map).bindPopup('Delivery in Progress');
    ...
    this.routeLine = L.polyline([startPoint, destination], { color: 'blue', weight: 4 }).addTo(this.map);
}
