            ...
            <table class="table-auto w-full border-collapse border border-gray-200 rounded-lg">
                ...
                <tbody>
                    ...
                    <tr class="bg-gray-100">
                        <td colspan="3" class="px-4 py-2 border border-gray-200 text-left font-bold">No Transaksi: {{ $transaction->transaction_id }}</td> <!-- Display Transaction ID -->
                    </tr>
                    @foreach($transaction->transactionItems as $item)
                    <tr class="bg-gray-50">
                        <td class="px-4 py-2 border border-gray-200">{{ $item->product->product_name }}</td>
                        <td class="px-4 py-2 border border-gray-200 text-center">{{ $transaction->created_at }}</td>
                        <td class="px-4 py-2 border border-gray-200 text-center">{{ $transaction->status_pengiriman }}</td> <!-- Empty for button -->
                    </tr>
                    @endforeach
                    @endforeach
                </tbody>
